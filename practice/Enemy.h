#pragma once

#include<string>

class Enemy
{
private:
	std::string name;
	int health;
	int attack;
	int defence;
	int speed;

public:
	Enemy();
	Enemy(std::string, int, int, int, int);
	void setName(std::string);
	void setHealth(int);
	void setAttack(int);
	void setDefence(int);
	void setSpeed(int);
	std::string getName();
	int  getHealth();
	int  getAttack();
	int  getDefence();
	int  getSpeed();

};

Enemy::Enemy()    // default constructor if I ever get all these values that means I did not pass the values that needed to be passed using the other constructor
{
	name = " ";
	health = 0;
	attack = 0;
	defence = 0;
	speed = 0;

}


Enemy::Enemy(std::string n, int h, int a, int d, int s)
{
	name = n;
	health = h;
	attack = a;					// each letter in parameter list is the first letter of the stat they go to. 
	defence = d;				// ex h stands for health // a stands for attack // d stands for defense // s stands for speed // n stands for name
	speed = s;

}

void Enemy::setName(std::string n)
{
	name = n;
}

void Enemy::setHealth(int h)
{
	health = h;
}

void Enemy::setAttack(int a)
{
	attack = a;
}

void Enemy::setDefence(int d)
{
	defence = d;
}

void Enemy::setSpeed(int s)
{
	speed = s;
}

std::string Enemy::getName()
{
	return name;
}

int Enemy::getHealth()
{
	return health;
}

int Enemy::getAttack()
{
	return attack;
}

int Enemy::getDefence()
{
	return defence;
}

int Enemy::getSpeed()
{
	return speed;
}